...
class MEXP_Anvato_Service extends MEXP_Service {
	...
	public function load() {
		...
		wp_enqueue_style(
			'mexp-service-anvato',
			ANVATO_URL . 'mexp/style.css',
			array( 'mexp' ),
			'0.1.5'
		);
		wp_enqueue_script(
			'mexp-service-anvato',
			ANVATO_URL . 'mexp/js.js',
			array( 'jquery', 'mexp' ),
			'0.1.5'
		);
	}
	...
	private function generate_shortcode( $video_id, $type = 'vod' ) {
		...
		$shortcode = '[anvplayer ' . ( 'vod' === $type ? 'video' : ( 'feed' === $type ? 'feed' : 'playlist' ) ) . '="' . esc_attr( $video_id ) . '"';
		if ( ! empty( $station ) ) {
	...
	public function tabs( array $tabs ) {
		$tabs[ ANVATO_DOMAIN_SLUG ] = array(
			'all' => array(
				'defaultTab'    => true,
				'text'          => _x( 'All', 'Tab title', 'wp_anvato' ),
				'fetchOnRender' => false,
			),
		);
	...
	public function labels( array $labels ) {
		$labels['wp_anvato'] = array(
			'insert'    => __( 'Insert Video', 'wp_anvato' ),
			'noresults' => __( 'No videos matched your search query.', 'wp_anvato' ),
			'title'     => __( 'Insert Anvato Video', 'wp_anvato' ),
			'loadmore'  => __( 'Load more videos', 'wp_anvato' ),
		);
