...
class MEXP_Anvato_Template extends MEXP_Template {
	...
	public function item($id, $tab) {
		?>
		<div id="mexp-item-<?php echo esc_attr($tab); ?>-{{ data.id }}" class="mexp-item-area <# if ( data.meta.type == 'playlist' ) { #>anvato-playlist-item<# }else { if ( data.meta.type == 'live') { #>anvato-channel-item<# }else{#>anvato-video-item<#}}#>" data-id="{{ data.id }}"  >
			<div class="mexp-item-container clearfix">
                                
				<div class="mexp-item-thumb" style="background-image: url({{ data.thumbnail }})" class="thickbox">
					<img src="<?php echo esc_url(ANVATO_URL . 'img/play.png') ?>" onclick="anv_preview('<?php echo $this->anv_settings['mcp']['id'] ?>', '{{ data.id }}', '{{ data.meta.type }}');" />
					<# if ( data.meta.duration  ) { #><span>{{ data.meta.duration }}</span><# } #>
				</div>
                                 
				<div class="mexp-item-main">
					<div class="mexp-item-content">
						<span class="anv-title">{{ data.content }}</span>
                                                <# if(data.meta.description) { #><span class="anv-desc">{{ data.meta.description }}</span><# } #>
					</div>
                                        <# if ( data.meta.video_count ) { #>
					<div class="mexp-item-meta">
						<span>
                                                    <#if ( data.meta.video_count == 0 ) {#> No Video in Playlist
                                                    <#}else{ #>{{ data.meta.video_count }} video<#if ( data.meta.video_count >1 ) {#>s<#}#> in playlist<# } #>
                                                </span>
					</div>
                                        <# } #>
	...
	public function search($id, $tab) 
        {   
            ...
            {
                $pref = json_decode( stripslashes_deep($_COOKIE['anv_user_preferences']) , TRUE );
            }
		?>
			...
			<input type="text" name="q" value="{{ data.params.q }}" class="mexp-input-text mexp-input-search" 
				   size="40" placeholder="<?php esc_attr_e('Search for videos', 'mexp'); ?>" >
                        <select name="station">
                            ...
                            {
                                echo "<option ".($pref['station'] === $item['label'] ? 'selected' :'')." value='{$item['id']}'>{$item['label']}</option>";
                            }
                            ?>
                        </select>
                        <select onchange="anv_type_select(this)" name='type'>
                                <option <?php echo ($pref['type'] === 'vod' ? 'selected' : '') ?> value='vod'>Video clips</option>
				<option <?php echo ($pref['type'] === 'playlist' ? 'selected' : '') ?> value='playlist'>Playlists</option>
				<option <?php echo ($pref['type'] === 'live' ? 'selected' : '') ?> value='live'>Live channel</option>
			</select>
