...
class Anvato_Library {
	...
	private function get_api_error( $response ) {
		...
		if ( is_object( $xml ) && ! empty( $xml->comment ) ) {
			return sprintf( esc_html( $xml->comment ) );
		} else {
			// Intentionally uncapitalized.
			return __( 'no error message provided', 'wp_anvato' );
		}
	}
	...
	private function request( $params ) {
		if ( ! $this->has_required_settings() ) {
			return new WP_Error(
				...
				__( 'The MCP URL, Public Key, and Private Key settings are required.', 'wp_anvato' )
			);
		}
		...
		if ( wp_remote_retrieve_response_code( $response ) === 200 ) {
			if ( $this->is_api_error( $response ) ) {
				return new WP_Error(
					'api_error',
					sprintf(
							/* translators: %s is replaced with api error message */
						__( '%s Please check your configuration parameters on Settings page.', 'wp_anvato' ),
		...
		return new WP_Error(
			...
			__( 'There was an error contacting Anvato.', 'wp_anvato' )
		);
	}
	...
	public function search( $args = array(), $output_type = 'clean' ) {
		if ( empty( $args['station'] ) ) {
			return new WP_Error(
				'missing_required_settings',
				__( 'Please select station.', 'wp_anvato' )
			);
		}
		...
		if ( ! is_object( $xml ) ) {
			return new WP_Error(
				'parse_error',
				__( 'There was an error processing the search results.', 'wp_anvato' )
			);
		}
